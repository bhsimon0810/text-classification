...
tf.flags.DEFINE_string("cell_type", "lstm", "Comma-separated filter sizes (default: '3,4,5')")
tf.flags.DEFINE_integer("hidden_size", 128, "Dimensionality of RNN cell(default: 128)")
...
def train():
    ...
    train = Dataset(filepath=FLAGS.train_data, num_class=FLAGS.num_class, sequence_length=FLAGS.sequence_length)
    valid = Dataset(filepath=FLAGS.valid_data, num_class=FLAGS.num_class, sequence_length=FLAGS.sequence_length)
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            train_op = tf.compat.v1.train.AdamOptimizer(FLAGS.learning_rate).minimize(rcnn.loss, global_step=global_step)
            acc, acc_op = tf.compat.v1.metrics.accuracy(labels=rcnn.labels, predictions=rcnn.predictions, name="metrics/acc")
            ...
            acc_summary = tf.compat.v1.summary.scalar("accuracy", rcnn.accuracy)
            ...
            for epoch in range(FLAGS.num_epoch):
                ...
                for batch in train.bacth_iter(FLAGS.batch_size, desc="Training", shuffle=True):
                    ...
                    _, step, summaries, loss, accuracy, _ = sess.run(
                        [train_op, global_step, train_summary_op, rcnn.loss, rcnn.accuracy, acc_op], feed_dict
                    )
                ...
                for batch in valid.bacth_iter(FLAGS.batch_size, desc="Validating", shuffle=False):
                    ...
                    summaries, loss, accuracy, _ = sess.run(
                        [valid_summary_op, rcnn.loss, rcnn.accuracy, acc_op], feed_dict
                    )
