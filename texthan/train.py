...
tf.flags.DEFINE_integer("att_size", 128, "Dimensionality of attention size (Default: 128)")
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            train_op = tf.train.AdamOptimizer(FLAGS.learning_rate).minimize(han.loss, global_step=global_step)
            acc, acc_op = tf.metrics.accuracy(labels=han.labels, predictions=han.predictions, name="metrics/acc")
            ...
            for epoch in range(FLAGS.num_epochs):
                ...
                for batch in train.bacth_iter(FLAGS.batch_size, desc="Training", shuffle=True):
                    ...
                    _, step, summaries, loss, accuracy, _ = sess.run(
                        [train_op, global_step, train_summary_op, han.loss, han.accuracy, acc_op], feed_dict
                    )
                ...
                for batch in valid.bacth_iter(FLAGS.batch_size, desc="Validating", shuffle=False):
                    ...
                    summaries, loss, accuracy, _ = sess.run(
                        [valid_summary_op, han.loss, han.accuracy, acc_op], feed_dict
                    )
