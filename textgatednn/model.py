...
class HieNet(object):
    def __init__(self, cell_type, hid_size, vocab_size, emb_size, num_classes, pretrained_embs,
                 l2_reg_lambda=0.0):
        ...
        with tf.variable_scope("word-encoder"):
            ...
            self.word_enc_outputs = self._last_relevant(tf.concat(word_enc_outputs, axis=2), word_length)
            self.word_level_outputs = tf.contrib.layers.dropout(self.word_enc_outputs, self.dropout_keep_prob, is_training=self.is_training)
        ...
        with tf.variable_scope("sent-encoder"):
            ...
            self.sent_enc_outputs = self._last_relevant(tf.concat(sent_enc_outputs, axis=2), self.sent_length)
            self.sent_level_outputs = tf.contrib.layers.dropout(self.sent_enc_outputs, self.dropout_keep_prob, is_training=self.is_training)
