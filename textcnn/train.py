...
tf.flags.DEFINE_integer("num_filter", 64, "Number of filters per filter size (default: 128)")
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            train_op = tf.compat.v1.train.AdamOptimizer(FLAGS.learning_rate).minimize(cnn.loss, global_step=global_step)
            acc, acc_op = tf.compat.v1.metrics.accuracy(labels=cnn.labels, predictions=cnn.predictions, name="metrics/acc")
            ...
            for epoch in range(FLAGS.num_epoch):
                ...
                for batch in train.bacth_iter(FLAGS.batch_size, desc="Training", shuffle=True):
                    ...
                    feed_dict = {
                        cnn.inputs: padded_docs,
                        cnn.labels: labels,
                        cnn.dropout_keep_prob: FLAGS.dropout_keep_prob
                    }
                    _, step, summaries, loss, accuracy, _ = sess.run(
                        [train_op, global_step, train_summary_op, cnn.loss, cnn.accuracy, acc_op], feed_dict
                    )
                ...
                for batch in valid.bacth_iter(FLAGS.batch_size, desc="Validating", shuffle=False):
                    ...
                    summaries, loss, accuracy, _ = sess.run(
                        [valid_summary_op, cnn.loss, cnn.accuracy, acc_op], feed_dict
                    )
