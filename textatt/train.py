...
def train():
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            train_op = tf.compat.v1.train.AdamOptimizer(FLAGS.learning_rate).minimize(rnn.loss, global_step=global_step)
            acc, acc_op = tf.compat.v1.metrics.accuracy(labels=rnn.labels, predictions=rnn.predictions, name="metrics/acc")
            ...
            for epoch in range(FLAGS.num_epoch):
                ...
                for batch in train.bacth_iter(FLAGS.batch_size, desc="Training", shuffle=True):
                    ...
                    padded_docs, masks, lengths = vectorize(docs, FLAGS.sequence_length)
                    feed_dict = {
                    ...
                    _, step, summaries, loss, accuracy, _ = sess.run(
                        [train_op, global_step, train_summary_op, rnn.loss, rnn.accuracy, acc_op], feed_dict
                    )
                ...
                for batch in valid.bacth_iter(FLAGS.batch_size, desc="Validating", shuffle=False):
                    ...
                    summaries, loss, accuracy, _ = sess.run(
                        [valid_summary_op, rnn.loss, rnn.accuracy, acc_op], feed_dict
                    )
