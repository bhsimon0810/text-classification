...
class TextRNN(object):
    ...
    def __init__(
        ...
        self.masks = tf.compat.v1.placeholder(tf.float32, [None, sequence_length], name="masks")
        self.lengths = tf.compat.v1.placeholder(tf.int32, [None], name="lengths")
        ...
        with tf.device('/cpu:0'), tf.compat.v1.variable_scope("embedding"):
            ...
            masks = tf.tile(tf.expand_dims(self.masks, -1), [1, 1, embedding_size])
            masked_inputs = self.embedded_inputs * masks
        ...
        with tf.compat.v1.name_scope("rnn"):
            ...
            outputs, _ = dynamic_rnn(cell=cell,
                                     inputs=masked_inputs,
                                     sequence_length=self.lengths,
                                     dtype=tf.float32)

        # attention Layer
        with tf.compat.v1.name_scope("attention"):
            self.att_outputs = attention(inputs=outputs, masks=self.masks, attention_size=hidden_size)

        # add dropout
        with tf.compat.v1.name_scope("dropout"):
            self.h_drop = tf.nn.dropout(self.att_outputs, self.dropout_keep_prob)
